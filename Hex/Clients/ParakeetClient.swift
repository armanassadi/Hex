...
actor ParakeetClient {
  ...
  private let vendorDirs = [
    // Our app-specific cache path convention (under XDG or <bundleIdentifier>/cache)
    "fluidaudio/Models",
    "FluidAudio/Models",
  ...
  private func candidateRoots() -> [URL] {
    ...
    let appCache = appSupport?.appendingPathComponent("\(HexCoreConstants.bundleIdentifier)/cache", isDirectory: true)
